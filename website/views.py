...
def one_user_history():
    ...
    if len(history_list) == 0:
        flash('you do not have travel history', category='error')
    return render_template('one_user_history.html', history_list=history_list)
...
def get_recommendation_list():
    user_found = db.users.find_one({'email': session['email']})
    flag_profile = user_found.get('age', None)
